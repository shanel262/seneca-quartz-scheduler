...
module.exports = function(options) {
  ...
  seneca.add({role: 'scheduler', cmd: 'register'}, function(args, done) {
    ...
    scheduler.on(taskId, function(data, callback) {
      debug('scheduler.on(%s) called', taskId);
      debug(data);
  ...
  seneca.add({role: 'scheduler', cmd: 'update'}, function(args, done) {
    ...
    scheduler.update(taskId, args.when, args.data, {key: args.jobId}, function(err, jobId) {
      seneca.log.debug('Updated job', taskId);
      debug('job updated, taskId: %s, jobId: %s', taskId, jobId);
      var scheduledTask = {
        when: args.when,
        data: args.data,
        taskId: taskId,
  ...
  seneca.add({role: 'scheduler', cmd: 'deregister'}, function(args, done) {
    ...
    scheduler.cancel(args.jobId, function(err, result) {
      ...
      seneca.log.debug('job canceled', result.taskId);
